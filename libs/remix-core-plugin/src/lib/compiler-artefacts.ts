...
export class CompilerArtefacts extends Plugin {
  ...
  onActivation () {
    ...
    this.plugin.on('bif-solidity', 'compilationFinished', (file, source, languageVersion, data, input, version) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source, input)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })

    this.plugin.on('vyper', 'compilationFinished', (file, source, languageVersion, data) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })

    this.plugin.on('lexon', 'compilationFinished', (file, source, languageVersion, data) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })

    this.plugin.on('yulp', 'compilationFinished', (file, source, languageVersion, data) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })

    this.plugin.on('solidityUnitTesting', 'compilationFinished', (file, source, languageVersion, data, input, version) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source, input)
      saveCompilationPerFileResult(file, source, languageVersion, data, input)
    })

    this.plugin.on('nahmii-compiler', 'compilationFinished', (file, source, languageVersion, data) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })

    this.plugin.on('hardhat', 'compilationFinished', (file, source, languageVersion, data) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })

    this.plugin.on('truffle', 'compilationFinished', (file, source, languageVersion, data) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })

    this.plugin.on('foundry', 'compilationFinished', (file, source, languageVersion, data) => {
      this.compilersArtefacts.__last = new CompilerAbstract(languageVersion, data, source)
      saveCompilationPerFileResult(file, source, languageVersion, data)
    })
  }
  ...
  async _populateAllContractArtefactsFromFE (path, contractName, contractArtefacts) {
    const dirList = await this.plugin.call('fileManager', 'dirList', path)
    if(dirList && dirList.length) {
      for (const dirPath of dirList) {
        ...
        if(dirPath === path + '/artifacts' && await this.plugin.call('fileManager', 'exists', dirPath + '/build-info')) {
          const buildFileList = await this.plugin.call('fileManager', 'fileList', dirPath + '/build-info')
          // process each build-info file to populate the artefacts for contractName
          for (const buildFile of buildFileList) {
            let content = await this.plugin.call('fileManager', 'readFile', buildFile)
  ...
  async getCompilerAbstract (file) {
    ...
    const path = await this.plugin.call('fileManager', 'getPathFromUrl', file)
  ...
  async getContractDataFromAddress (address) {
    const code = await this.plugin.call('blockchain', 'getCode', address)
