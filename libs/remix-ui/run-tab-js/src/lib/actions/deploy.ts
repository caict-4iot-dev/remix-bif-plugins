...
export const getSelectedContract = (contractName: string, compiler: any): ContractData => {
  ...
  const { abi, deployedBytecode } = compiler;
  ...
  return {
    ...
    getConstructorInterface: () => {
      return txHelper.getConstructorInterface(abi);
    },
    getConstructorInputs: () => {
      const constructorInteface = txHelper.getConstructorInterface(abi);

      return txHelper.inputParametersDeclarationToString(constructorInteface.inputs);
    },
    isOverSizeLimit: async (args: string) => {
      return {
        overSizeEip3860: false,
        overSizeEip170: false,
      };
    },
...
export const terminalLogger = (plugin: RunTab, view: string) => {
...
export const confirmationHandler = (plugin: RunTab, dispatch: React.Dispatch<any>, confirmDialogContent: MainnetPrompt, network, tx, gasEstimation, continueTxExecution, cancelCb) => {
  ...
  dispatch(displayNotification('Confirm transaction', content, 'Confirm', 'Cancel', () => {
    plugin.config.setUnpersistedProperty('doNotShowTransactionConfirmationAgain', plugin.REACT_API.confirmSettings)
...
export const runTransactions = async (
  ...
  await bif.runOrCallContractMethod(
    ...
    (returnValue) => {
      ...
      dispatch(setDecodedResponse(instanceIndex, returnValue, funcIndex));
    },
