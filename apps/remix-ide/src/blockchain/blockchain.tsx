...
export class Blockchain extends Plugin {
    ...
    const web3Runner = new TxRunnerWeb3(
      {
        ...
        personalMode: () => {
          return this.getProvider() === 'web3' ? this.plugin.config.get('settings/personal-mode') : false
        }
      },
  ...
  setupProviders() {
    ...
    this.providers.web3 = new NodeProvider(this.executionContext, this.plugin.config)
  }
  ...
  async deployProxy(proxyData, implementationContractObject) {
    const proxyModal = {
      ...
      cancelFn: () => {
        this.plugin.call('notification', 'toast', cancelProxyMsg())
        _paq.push(['trackEvent', 'blockchain', 'Deploy With Proxy', 'cancel proxy deployment'])
      },
      hideFn: () => null
    }
    this.plugin.call('notification', 'modal', proxyModal)
  }
  ...
  async runProxyTx(proxyData, implementationContractObject) {
    ...
    const finalCb = async (error, txResult, address, returnValue) => {
      if (error) {
        ...
        return this.plugin.call('terminal', 'logHtml', log)
      }
      ...
      this.plugin.call('udapp', 'addInstance', addressToString(address), implementationContractObject.abi, implementationContractObject.name)
    }
  ...
  async upgradeProxy(proxyAddress, newImplAddress, data, newImplementationContractObject) {
    const upgradeModal = {
      ...
      cancelFn: () => {
        this.plugin.call('notification', 'toast', cancelUpgradeMsg())
        _paq.push(['trackEvent', 'blockchain', 'Upgrade With Proxy', 'proxy upgrade cancel click'])
      },
      hideFn: () => null
    }
    this.plugin.call('notification', 'modal', upgradeModal)
  }
  ...
  async runUpgradeTx(proxyAddress, data, newImplementationContractObject) {
    ...
    const finalCb = async (error, txResult, address, returnValue) => {
      ...
      this.plugin.call('udapp', 'addInstance', addressToString(proxyAddress), newImplementationContractObject.abi, newImplementationContractObject.name)
    }
    this.runTx(args, confirmationCb, continueCb, promptCb, finalCb)
  }
  ...
  async saveDeployedContractStorageLayout(contractObject, proxyAddress, networkInfo) {
    ...
    const hasPreviousDeploys = await this.plugin.call('fileManager', 'exists', `.deploys/upgradeable-contracts/${networkName}/UUPS.json`)
    // TODO: make deploys folder read only.
    if (hasPreviousDeploys) {
      const deployments = await this.plugin.call('fileManager', 'readFile', `.deploys/upgradeable-contracts/${networkName}/UUPS.json`)
      ...
      if (proxyDeployment) {
        ...
        const hasPreviousBuild = await this.plugin.call('fileManager', 'exists', `.deploys/upgradeable-contracts/${networkName}/solc-${oldImplementationAddress}.json`)

        if (hasPreviousBuild) await this.plugin.call('fileManager', 'remove', `.deploys/upgradeable-contracts/${networkName}/solc-${oldImplementationAddress}.json`)
      }
      ...
      await this.plugin.call('fileManager', 'writeFile', `.deploys/upgradeable-contracts/${networkName}/UUPS.json`, JSON.stringify(parsedDeployments, null, 2))
    } else {
      await this.plugin.call(
        'fileManager',
        'writeFile',
      ...
      */
      ...
      if (isVM) {
        ...
        if (hhlogs && hhlogs.length) {
          ...
          this.plugin.call('terminal', 'logHtml', finalLogs)
        }
        ...
        if (execResult) {
          ...
          const compiledContracts = await this.plugin.call('compilerArtefacts', 'getAllContractDatas')
